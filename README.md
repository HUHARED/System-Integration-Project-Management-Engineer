...
1. 总价合同
   1. 固定总价，一开始就确定价格，不允许改变
2. 总价加激励合同
   1. 允许一定绩效偏差，对实现既定目标给予奖励
3. 总价加经济价格调整合同
   1. 卖方履约要花费很长时间（数年），则使用该合同。
4. 订购单
   1. 非大量采购标准化产品，通常买方直接填写卖方提供的订购单，卖方供货。
   2. 不需要谈判，又称单边合同。
   3. 好像就是类似个人消费者买东西嘛。
5. 成本补偿合同
   1. 发包人向承包人支付全部合法成本之后，按照事先约定的某种方式再外加一笔作为利润。
   2. 适用于
      1. 需要立刻开展工作的项目
...
* 为了管理控制而特别留出的预算，用来应对项目范围中不可见的工作。
* 使用前需要得到高层批准。
...
###### 3.2.6.4.3.6. 【确认范围】过程、【质量控制】过程的区别
